...
class Inlet():
    ...
    def __init__(self, parent, index, sources=None, midpoints=None, types=None):
        ...
        self._sources = sources if sources is not None else []     #list of Outlets
        self._midpoints = midpoints if midpoints is not None else [] #list of midpoints (this info only in inlet of outlet, inlet pair) 
        self._types = types if types is not None else []         #allowed connection types 
